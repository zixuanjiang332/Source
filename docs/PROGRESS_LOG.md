...
- 当前拳头版正式动作帧还未导出，运行时暂时回退到旧的刀系条带播放。
- 拳头动作生产包已齐，但正式拳头 spritesheet 仍未生成并接入 Godot。
- 当前已接入的是“过渡拳头条带”，力量感和动作语义已比旧刀系贴近空手近战，但仍需要更高质量正式拳头帧替换。
- 当前部分 AI 参考帧仍存在双机械臂倾向，后续出图必须统一改用 `YUAN_APPEARANCE_LOCK.md`。
- `punch_skill` 已修到单机械右臂版本，但其余后续参考帧仍需继续按外貌锁逐批替换。
