...
- 当前拳头 runtime 已接入，但部分帧存在脸部变黑、腿部越界后回绕、蓝色命中特效缺失和动作过硬的问题，需要继续修正帧归一化与打击帧处理。
...
- 确认第一版可录屏路线的场景节奏。
- 生成并接入拳头版 `combat_idle/combat_run/punch_1/punch_2/punch_3/punch_skill` 高质量帧图。
- 先按生产包生成关键姿势候选，再补全中间帧和运行时条带。
- 用更高质量正式拳头帧替换当前过渡版 `combat_* / punch_*` runtime 资源，并做 GUI 手感验收。
...
| 初始拳头玩法切片 | Codex / 程序 | Review | 已切换默认武器资源、接入过渡拳头 runtime 资源并清理旧刀系战斗逐帧，待更高质量正式拳头帧替换 |
