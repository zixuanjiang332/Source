...
- 新增拳头开局攻击资源 `fist_jab_1`、`fist_cross_2`、`fist_breaker_3`、`fist_drive_step` 与 `initial_fists.tres`。
- 新增拳头动作出图规格，明确 `combat_idle`、`combat_run`、`punch_1`、`punch_2`、`punch_3`、`punch_skill` 的帧数与力量感要求。
- 新增 `FIST_ANIMATION_PRODUCTION_PACK.md`，把拳头开局动作拆成逐动作、逐关键帧、逐提示词的生产包。
- 新增拳头动作逐帧提示词与筛图审核表，方便按帧生成和统一评审。
- 新增“源”主角外貌锁定提示词，明确只有右臂为机械义体，左臂必须保持人类手臂。
- 新增 Batch 02 技能帧替换清单与参考图，开始单独迭代 `punch_skill` 的单机械右臂版本。
- 新增拳头动作逐帧 `FRAME_SHOTLIST.csv`，用于按帧追踪候选、返修和接入状态。
- 新增拳头动作一致性锁和 Batch 01 最终提示词，降低批量出图时的人物漂移问题。
- 新增拳头运行时构建/部署/启动脚本，支持新帧落地后清理旧战斗帧并切到新 SpriteFrames。
- 新增过渡拳头条带生成脚本，基于旧早期克隆体战斗帧剥离刀光并拼成 `combat_* / punch_*`，用于先行实装测试。
- 新增正式拳头条带模板生成脚本与模板 PNG，方便后续在正确尺寸和基准线上直接重绘正式高质量动作。
- 新增过渡拳头动作 paintover guide 生成脚本与 guide PNG，方便在现有运行时节奏上直接覆盖重绘正式帧。
- 新增拳头关键帧看板和逐张 strip 重绘任务单，方便直接判断先改哪几帧最值。
- 新增 Batch 01 的 9 帧逐帧重绘任务单与最终提示词，把正式拳头重绘收敛到最值的一小批帧。
- 新增 Batch 01 的 3x3 视觉参考提示词，用于先统一拳头连段的力量层级和角色体态方向。
- 新增 Batch 01 的替换脚本、投放目录 README 和替换清单，形成重绘后直接实装部署的闭环。
- 新增两张已落地的拳头视觉参考图，并登记到项目文档，供 Batch 01 重绘前统一单帧力量方向。
...
- 玩家默认武器从匕首切到拳头原型；当前逻辑先请求拳头动画标签，缺失时回退到旧的刀系条带，方便逐步替换新帧。
- 玩家动画控制器改为优先播放真实 `combat_*` / `punch_*` 标签，旧 `idle/run/atk_*/skill` 只在新帧缺失时兜底。
- 玩家场景现已切到 `player_yuan_runtime_frames.tres`，运行时使用过渡拳头动作集，旧 `atk_* / skill` 逐帧 PNG 已从原逐帧目录清理。
- 将 PR 文档同步要求纳入团队协作流程。
...
- `WeaponData` 增加终结技显示名、能量消耗、长按时间和终结技攻击链。
- `VfxCatalog` 从少量硬编码 scene 字段扩展为可登记多条 VFX/SFX entry，同时保留旧 fallback。
- HUD 增加 `HOLD K` 终结技能量状态提示。
...
- 修复拳头 runtime 初次切换后的资源接入问题；完成导入后 headless 启动通过。
- 修正 `punch_skill` 参考帧中的双机械臂偏差，替换为单机械右臂版本并重新部署 runtime。
...
- 同步拳头动作生产包入口与主角动作交付说明。
- 同步“源”主角单机械右臂外貌锁定规则到关键帧、逐帧和重绘提示词。
